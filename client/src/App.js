...
//allday 이거 지웠고, 
function App() {  //clonedeep 사실 필요없음, 게다가 useEffect에서 db에서 데이터 받을꺼라서 없어도됨
  ...
  .then(response=>{
    console.log(response.data.event)  
      let arr = response.data.event.map((ele)=>{   //map 쓰는법
  ...
  //   useEffect(()=>{
  //   const arr = []      //빈 배열을을 만들어주고
  //   axios
  //   .post("/api/getEvent")         
  //   .then(response=>{
  //     console.log(response.data)  
  //     response.data.event.forEach(element => arr.push(element)) //forEach로 뽑아서 빈 배열인 arr에 푸쉬해주고
  //     console.log(arr)
  //     setEvents(arr)  //setEvents에 푸쉬가 끝난 arr을 넣어주면 끝!
  //   })    
  // },[])
